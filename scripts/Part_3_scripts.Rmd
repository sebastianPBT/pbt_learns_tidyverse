...
ggsave("../Output/betterplot.pdf", plot = plot,scale = 1, width = 10, height = 10, units = c("cm"), dpi = 600)
```
...
str(mtcars)# How big is this dataset?
```
...
```{r Patchwork Ex5}
p1 + grid::textGrob('I like graphs')
```
...
```{r}
# Specify labels
plot_grid(p1, p2, p3, ncol = 2, nrow = 2, labels = c('a', 'b', 'c'))
# Auto assign lables
plot_grid(p1, p2, p3, labels = "AUTO")
# Setting label characterisitics
final_plot<- plot_grid(p1, p2, p3, labels = "AUTO", label_size = 20, label_fontface = 'italic')
#Now Save!
library(ggplot2)
ggsave("../Output/final_plot.pdf", plot = final_plot,scale = 1, width = 10, height = 10, units = c("cm"), dpi = 600)
```
